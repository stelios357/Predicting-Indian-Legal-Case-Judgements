...
#        tt=input()
        ...
        validData =  convertCoordinateDataIntoList(df_coords, SplitYearTrain-9, SplitYearValid)
        testData =  convertCoordinateDataIntoList(df_coords, SplitYearValid-9, end_Year)
...
def getLSTM(dataR_shape, n_filters):
    
    i1 = keras.layers.Input(shape = (dataR_shape[1],1)) #length of seq == 5 // instead of 1 -> 768
#    i1_reshape = keras.layers.Reshape((dataR_shape[1], 1))(i1)
    i2 = keras.layers.Input(shape=[2]) # only 1 ip
#    c1 = keras.layers.Conv1D(filters=n_filters, kernel_size=5, strides=1, padding= 'causal', activation='relu', input_shape=(dataR_shape[1],1))(i1_reshape)
 #   c2 = keras.layers.Conv1D(filters=n_filters, kernel_size=5, strides=1, padding='causal', activation='relu')(c1)
    h1 = keras.layers.LSTM(30, return_sequences=True)(i1)
    d1 = keras.layers.Dropout(rate=0.2)(h1)
    h2 = keras.layers.LSTM(30)(d1)
#    p1 = keras.layers.GlobalAveragePooling1D()(d1)
    concat = keras.layers.Concatenate()([i2,h2]) # comment out
    output = keras.layers.Dense(1)(concat) #make sure of output # or add activation fn

    return keras.models.Model(inputs=[i1, i2], outputs = [output]) #only i1

def compileAndFitModel(dataR, dataC, model):
    
    model.compile(loss='mse', optimizer=keras.optimizers.Adam(), metrics=['mae','mse'])
    checkpoint_cb = keras.callbacks.ModelCheckpoint(filepath="bestLSTM_WRD.h5", monitor='val_loss', save_best_only=True)
    history = model.fit((dataR.trainX, dataC.trainX), dataR.trainY, epochs=100, validation_data=((dataR.validX, dataC.validX), dataR.validY), callbacks=[checkpoint_cb], batch_size=32, verbose=2)
    return keras.models.load_model("bestLSTM_WRD.h5")
...
#    normalizeDataset(dataset, dataRainfall, 0, 100)
    ...
    model = getLSTM(dataRTrainXShape, n_filters=100)
